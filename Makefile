...
CFLAGS  += $(WARN) $(OPT) $(SAN) $(EXTRA_CFLAGS) -MMD -MP
LDFLAGS += $(SAN)
...
HEADERS  := $(wildcard core/*.h host/*.h tests/*.h)
DEPS     := $(CORE_OBJ:.o=.d) $(HOST_OBJ:.o=.d) $(BUILD)/host/flsim_main.d
...
$(BUILD)/test_%: tests/test_%.c $(CORE_OBJ) $(HOST_OBJ) $(HEADERS)
	@mkdir -p $(BUILD)
	$(CC) $(CFLAGS) $< $(CORE_OBJ) $(HOST_OBJ) $(LDFLAGS) -o $@

# Header dependencies generated by -MMD so a changed struct layout rebuilds every user.
-include $(DEPS)
